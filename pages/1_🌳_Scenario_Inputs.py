...
def page() -> None:
    ...
    with st.form("project_form"):
        ...
        with col1:
            ...
            tCO2_price = st.number_input("CO₂ credit price (EUR/t)", min_value=0.0, value=float(scn.co2_price_per_tonne), step=1.0)
            Op_Cost = st.number_input("Other costs (EUR/ha per year)", min_value=0.0, value=float(scn.other_costs_per_ha_per_year), step=10.0)
            Extra_Rev = st.number_input("Other revenue (EUR/ha per year)", min_value=0.0, value=float(scn.other_rev_per_ha_per_year), step=10.0)
            
            #Logistics
            st.subheader("Logistics Farm to Production")
            Trailer_payload = st.number_input("Load for each trailer (t)", min_value=1, max_value=120, value=scn.logistics.trailer_payload_t, step=1)
            Max_distance = st.number_input("Maximum distance (km)", min_value=1, max_value=1000, value=scn.logistics.transport_distance_km, step=1)
            Cost_per_km = st.number_input("Gas/Employee/Cost km (Eur)", min_value=1, max_value=80, value=scn.logistics.transport_cost_per_km, step=1)
            truck_emiss_CO2 = st.number_input("Emission from logistics (tCO2)", min_value=0, max_value=120, value=scn.logistics.truck_emission_kg_per_tkm, step=1)
        with col2:
            #RootExtraction
            st.subheader("Paulownia Root Extraction")
            Price_Oleic_kg = st.number_input("Retail Price of Oleic Acid (kg)", min_value=0.0, max_value=1000.0, value=scn.extraction.price_oleic_eur_per_kg, step=0.1)
            Price_Theobromine = st.number_input("Retail Price of Theobromine (kg)", min_value=0.0, max_value=5000.0, value=scn.extraction.price_theobromine_eur_per_kg, step=0.1)
            Price_Myzelbooster = st.number_input("Retail Price of MyzelBooster (L)", min_value=0.0, max_value=5000.0, value=scn.extraction.price_extract_eur_per_L, step=0.1)
            #Substrate
            st.subheader("Substrate composition of Material")
            root_percentage = st.number_input("Per_roots (%)", min_value=0.0, max_value=1.0, value=scn.substrate.root_fiber_share, step=0.01)
            other_percentage = st.number_input("Per_substrates+additives (%)", min_value=0.0, max_value=1.0, value=scn.substrate.other_dry_share, step=0.01)
            kWh_per_t_sterilized = st.number_input("kWh per ton sterilized", min_value=0.0, max_value=100.0, value=scn.substrate.sterilize_kWh_per_t_substrate, step=0.01)
            
        with col3:    
            #PlateParameters
            st.subheader("Parameters of Plate Manufacturing")
            # --- Plate Parameters (INT widgets) ---
            ...
            Employees_Automation = st.slider("Min Employees at Full automation", min_value=0, max_value=100, value= scn.labor.min_automation_employees, step = 1 )
            Employees_NON_Automation = st.slider("Max Employees in Manual Labor", min_value=0, max_value=100, value= scn.labor.jobs_per_shift_high , step = 1 )
            Shifts_per_day = st.number_input("Shifts per day", min_value=0, max_value=4, value=scn.labor.shifts_per_day, step=1)
            #End_of_Life (EoL)
            st.subheader("End-of-Life of Material")
            Recovered_Perc = st.slider("Percentage recovered yearly", min_value=0.0, max_value=1.0, value= float(scn.eol.recovered_plate_frac), step = 0.01 )
            CarbonPrice = st.slider("Carbon Price at the year of exchange", min_value=0.0, max_value=100.0, value=float(scn.eol.carbon_price_mid_eur), step = 0.01 )
