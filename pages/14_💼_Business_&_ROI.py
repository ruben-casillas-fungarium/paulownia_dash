...
def page() -> None:
    ...
    with st.form("biz_inputs"):
        ...
        plate_price = st.number_input("Plate selling price (€/plate)", min_value=0.0, value=float(getattr(getattr(scn, "plates", scn), "plate_price_eur", 12.0)), step=0.1)
        plate_cost = st.number_input("Plate manufacturing cost (€/plate)", min_value=0.0, value=float(getattr(getattr(scn, "plates", scn), "plate_cost_eur", 3.0)), step=0.1)
        eps_price = st.number_input("EPS competitor price (€/plate)", min_value=0.0, value=float(getattr(getattr(scn, "plates", scn), "competitor_eps_price_eur", 12.0)), step=0.1)
        eps_cost = st.number_input("EPS competitor cost (€/plate)", min_value=0.0, value=float(getattr(getattr(scn, "plates", scn), "competitor_eps_cost_eur", 6.0)), step=0.1)
        st.markdown("---")
        ...
        coinvest_share = st.slider("Investor co‑investment share of project", 0.0, 1.0, 0.20, 0.01)
        submitted = st.form_submit_button("Apply")
    alloc = _alloc_normalize({"farmers": to_farmers/100, "employees": to_employees/100, "company": to_company/100, "investors": to_investors/100})
    ...
    total_revenue = rev_plates + rev_extract + float(df_join.get("wood_rev", pd.Series([0])).sum()) + float(df_join.get("co2_rev", pd.Series([0])).sum())
    # Costs: manufacturing + known costs in joined
    known_costs = float(df_join.get("water_cost", pd.Series([0])).sum()) + float(df_join.get("opex", pd.Series([0])).sum()) + float(df_join.get("transport_cost_eur", pd.Series([0])).sum()) + float(df_join.get("additives_cost_eur", pd.Series([0])).sum()) + float(df_join.get("inoculum_cost_eur", pd.Series([0])).sum())
    ...
    st.plotly_chart(px.pie(alloc_series.rename("€").reset_index(), names="index", values="€", title="Profit allocation (normalized)"), width="stretch")
    ...
    fig_inv = px.line(x=list(range(len(investor_cf))), y=np.cumsum(investor_cf), markers=True, labels={"x":"Year index","y":"Cumulative €"}, title="Investor cumulative cash")
    ...
    st.plotly_chart(px.bar(df_cmp, x="product", y="margin_per_plate", title="Margin per plate comparison", labels={"margin_per_plate":"€/plate"}), width="stretch")
    ...
    st.download_button("Download Business summary CSV", df_summary.to_csv(index=False).encode(), "business_summary.csv", "text/csv")
